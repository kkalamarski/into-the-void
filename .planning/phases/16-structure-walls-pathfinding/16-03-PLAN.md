...
must_haves:
  truths:
    ...
    - "Wall heights vary by biome/type (e.g., ICE_WALL taller than VOID_WALL)"
  artifacts:
  ...
  key_links:
    ...
    - from: "packages/world-gen/src/generation/structures.ts"
      to: "getWallHeight function"
      via: "height lookup by tile ID"
      pattern: "getWallHeight\\(wallTileId\\)"
---

<objective>
Add procedural wall generation to world-gen with per-type height variation.

Purpose: Generate structure walls deterministically from seed, place them using noise threshold, connect with line segments. Different wall types have different heights for visual variety and gameplay.
Output: New structures.ts module, integrated into chunk generation, ChunkData.structures populated with variable-height walls.
</objective>
...
<task type="auto">
  <name>Task 1: Create structures.ts for procedural wall generation with per-type heights</name>
...
- WALL_HEIGHTS lookup table maps each tile ID to its specific height above terrain
- getWallHeight(wallTileId) returns the configured height for the wall type
- Crystal formations are tallest (5), lava/toxic pools lowest (2)
- createWallSegment calls getWallHeight AFTER determining wallTileId
- Default height of 3 used if unknown tile ID (defensive)
...
Verify WALL_HEIGHTS contains entries for all wall tile types.
  </verify>
  <done>
structures.ts exists with generateStructures function.
getWallHeight function returns different heights per tile type.
...
5. Height variation test: Verify crystal_caves biome walls have height 5, toxic_wastes have height 2
</verification>

<success_criteria>
- structures.ts exists with generateStructures function
- getWallHeight function returns per-type heights (not constant)
- CRYSTAL_FORMATION walls are height 5, TOXIC_POOL walls are height 2
