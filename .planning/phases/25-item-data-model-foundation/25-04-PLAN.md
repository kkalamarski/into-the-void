...
<task type="auto">
  <name>Task 1: Add @into-the-void/items dependency and create validation functions</name>
  <files>
    packages/game-logic/package.json
    packages/game-logic/src/inventory/validation.ts
  </files>
  <action>
First, add the items package as a dependency to game-logic.

**packages/game-logic/package.json** - Add to dependencies:

```json
"dependencies": {
  "@into-the-void/items": "workspace:*",
  // ... existing dependencies
}
```

Then create `packages/game-logic/src/inventory/validation.ts` with pure validation functions that mirror the pattern from `movement/validation.ts`.
...
<success_criteria>
- [ ] packages/game-logic/package.json has "@into-the-void/items": "workspace:*" dependency
