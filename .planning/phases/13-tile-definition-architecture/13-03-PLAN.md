...
<task type="auto">
  <name>Task 3: Update chunk generation to include heights</name>
  <files>
    packages/world-gen/src/generation/chunk.ts
  </files>
  <action>
Update generateChunkData to include heights in its output:
...
2. Update generateChunkData function to include heights and structures:

Find where generateTerrain is called and update to capture heights:
```typescript
const { tiles, heights, collisions } = generateTerrain(worldSeed, chunkX, chunkY, biome);
```

Update ChunkData return to include heights and empty structures:
```typescript
...
```
  ...
  <verify>
    - `grep "heights" packages/world-gen/src/generation/chunk.ts` shows heights usage
    - `grep "structures" packages/world-gen/src/generation/chunk.ts` shows structures array
    - `pnpm exec nx run world-gen:lint` passes
    - `pnpm exec nx run world-gen:build` succeeds
  </verify>
  <done>
    - generateChunkData returns complete ChunkData with heights and structures
...
- generateChunkData outputs complete ChunkData with heights and structures
